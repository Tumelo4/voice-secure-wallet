...
test("readiness state keeps service and validation evidence", () => {
  ...
  assert.equal(readinessSelectors.totalPassingTests(state), 131);
  assert.equal(readinessSelectors.activePhase(state).name, "Mobile Screen Commands");
