...
test("readiness state keeps service and validation evidence", () => {
  ...
  assert.equal(readinessSelectors.totalPassingTests(state), 85);
  assert.equal(readinessSelectors.activePhase(state).name, "API Runtime Boundary");
