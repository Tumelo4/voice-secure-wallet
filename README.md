...
| Terraform AWS baseline | Static tests verify required Terraform files, private networking, KMS rotation, MSK TLS/IAM and broker config, RDS HA/PITR, Redis encryption, S3 object lock, and no committed secret values; `scripts/test-terraform-aws-baseline.sh` runs `terraform fmt -check`, `terraform init -backend=false`, and `terraform validate` when Terraform is installed. |
