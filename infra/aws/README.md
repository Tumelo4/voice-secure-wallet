...
- `environments/production-reference/`: hardened reference using remote state,
  ...
  lock.
- `environments/staging/`: cost-controlled deployable staging composition using
  an isolated remote-state key and staging-only resource names.
...
2. Initialize `environments/staging/` or `environments/production-reference/`
   against its isolated backend key.
...
and validate bootstrap, demo, staging, and production-reference independently.
