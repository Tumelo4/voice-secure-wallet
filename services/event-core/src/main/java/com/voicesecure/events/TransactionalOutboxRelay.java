...
public final class TransactionalOutboxRelay {
    ...
    public RelayResult relayOnce() {
        ...
        for (OutboxMessage message : claimed) {
            try {
                ...
                store.markFailed(message.envelope().eventId(), workerId, clock.instant(), error, retryDelay);
                failed++;
            }
        }
        return new RelayResult(claimed.size(), published, failed);
    }
    ...
    public record RelayResult(int claimedCount, int publishedCount, int failedCount) { }
}
